...
def render(time, x, y, xLength: int, yLength: int, deformation = 1.0, colorRGB = [0.0, 0.0, 0.0]):
...
def main():
    ...
    color = [getRandom(), getRandom(), getRandom()]
    startup()
    render(glfwGetTime(), 8, -80, 90, 24, 1.3)
    glfwSwapBuffers(window)
    while not glfwWindowShouldClose(window):
        render(glfwGetTime(), 8, -80, 90, 24, 0.5, color)
        glfwSwapBuffers(window)
        glfwPollEvents()
    shutdown()
