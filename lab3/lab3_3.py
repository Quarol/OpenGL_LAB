...
def render(time, vertices : list, colors : list, N : int, angle):
...
def initializeColors(N : int):
    colors = [[[0] * 3 for i in range(N)] for j in range(N)]

    for i in range(N):
        for j in range(N):
                colors[i][j][0] = random.uniform(0.0, 1.0)
                colors[i][j][1] = random.uniform(0.0, 1.0)
                colors[i][j][2] = random.uniform(0.0, 1.0)
...
def main():
    ...
    colors = initializeColors(N)

    startup()
    while not glfwWindowShouldClose(window):
        render(glfwGetTime(), vertices, colors, N, glfwGetTime() * 45/math.pi)
